...
-- Recreate the handle_new_user function with proper specifications
...
BEGIN
  BEGIN
    ...
    BEGIN
      RAISE NOTICE 'handle_new_user: Extracted role: %', user_role;
      
      -- Insert the specified role for new users (avoid duplicates)
      RAISE NOTICE 'handle_new_user: Inserting user role...';
      INSERT INTO public.user_roles (user_id, role)
      VALUES (NEW.id, user_role)
      ON CONFLICT (user_id, role) DO NOTHING;
      RAISE NOTICE 'handle_new_user: User role inserted successfully';
      
      -- Insert user into users table if they provided metadata (avoid duplicates)
      RAISE NOTICE 'handle_new_user: Inserting user record...';
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = now();
      RAISE NOTICE 'handle_new_user: User record inserted successfully';
      
      -- If role is supplier, create supplier record
      IF user_role = 'supplier' THEN
        RAISE NOTICE 'handle_new_user: Creating supplier record...';
        ...
        RAISE NOTICE 'handle_new_user: Supplier record created successfully';
          
        -- Create default subscription for new supplier
        RAISE NOTICE 'handle_new_user: Creating subscription for supplier...';
        ...
        RAISE NOTICE 'handle_new_user: Subscription created successfully';
      END IF;
      
      RAISE NOTICE 'handle_new_user: Function completed successfully for user %', NEW.id;
      RETURN NEW;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'handle_new_user: ERROR occurred: % %', SQLERRM, SQLSTATE;
      RAISE NOTICE 'handle_new_user: Error details: %', SQLERRM;
      -- Re-raise the error so the transaction fails
      RAISE;
    END;
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'handle_new_user: OUTER ERROR occurred: % %', SQLERRM, SQLSTATE;
    -- Don't re-raise here to prevent auth failure
    RETURN NEW;
  END;
END;
$function$;
